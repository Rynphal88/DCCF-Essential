// src/components/ai/DccfAiChatWindow.tsx
"use client";
...
export function DccfAiChatWindow({ onClose }: Props) {
  const { position, size, onMouseDownDrag, onMouseDownResize, resetWindow } =
  ...
  const handleSend = () => {
    ...
    // TODO: wire to AI orchestrator
    setInput("");
  };

  const handleSmartAction = (type: "next" | "refine" | "big-picture") => {
    // TODO: connect to specialised prompts
  ...
  return (
    <div
      ...
      className="flex flex-col overflow-hidden rounded-3xl border border-slate-700/60 bg-slate-900/95 text-slate-50 shadow-2xl shadow-sky-500/30 backdrop-blur-xl"
    >
      {/* Header (drag handle) */}
      <div
          ...
          >
            ...
            >
              ...
              <span className="mt-1 block text-[10px] text-slate-300/70">
                {m.timestamp} ·{" "}
                {m.role === "assistant" ? "offline-hybrid" : "you"}
              </span>
            </div>
          </div>
        ))}
